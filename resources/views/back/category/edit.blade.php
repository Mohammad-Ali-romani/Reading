...
<div class="container-fluid mt-4 ">
        <div class="bg-light p-1 text-left box-tool">
                <form action="{{route('category.update',$categoryOne->id)}}" method="post" class="mb-0 row" >
                    @csrf
                    @method('PUT')
                    <div class="ml-5 mt-1 form-group">
                        <input name="name" type="text" class="form-control input-me" value="{{$categoryOne->name}}" placeholder="Name Category">
                    </div>
                    <button class="btn btn-primary ml-4">Edit</button>
                </form>
        </div>
